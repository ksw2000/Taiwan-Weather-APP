...
Future<dynamic> getForecastWeather(city) async {
  var url = "https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=${_authorization}&locationName=${city}";
    ...
    if (jsonResponse["success"] == "true") {
      ...
      for (var j = 0; j < 3; j++) {
        for (var i = 0; i < parameter.length; i++) {
          ret[parameter[i]["elementName"]] = parameter[i]["time"][j]["parameter"];
        }
        retList.add(ret);
      }
      return retList;
...
Future<dynamic> getCurrentWeather(city) async {
  var url = "https://opendata.cwb.gov.tw/api/v1/rest/datastore/O-A0003-001?Authorization=${_authorization}&locationName=${city}";
  var response = await http.get(url);
  if (response.statusCode == 200) {
    var jsonResponse = convert.jsonDecode(response.body);
    if(jsonResponse["success"] == "true"){
      var weatherElement = jsonResponse["records"]["location"][0]["weatherElement"];

      var len = weatherElement.length;
      var ret = new Map();
      for(var i=0; i<len; i++){
        ret[weatherElement[i]["elementName"]] = weatherElement[i]["elementValue"] as String;
      }
      return ret;
    }
  }
